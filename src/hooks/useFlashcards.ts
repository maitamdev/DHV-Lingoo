...
export function useFlashcards() {
  ...
  useEffect(() => {
    ...
    async function loadCards() {
      try {
        ...
        if (saved && !cancelled) setRevealed(JSON.parse(saved));
        ...
        if (!words?.length) { if (!cancelled) { setError('No vocabulary'); setLoading(false); } return; }
  ...
  return { cards, revealed, loading, error, today, openedCount, allOpened, totalXP, aiGenerated, topic, revealCard };
}
